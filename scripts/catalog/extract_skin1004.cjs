...
const targetBrand = 'Skin1004';
const brandFile = path.join(__dirname, 'src/data/brands/skin1004.ts');
...
scrapedFiles.forEach(file => {
  ...
  for (let i = 1; i < blocks.length; i += 2) {
    ...
    // Check for Skin1004 variation (Skin1004, SKIN1004)
    if (block.toLowerCase().includes("brand: 'skin1004'") || block.toLowerCase().includes('brand: "skin1004"')) {
      try {
...
fs.writeFileSync('extracted_skin1004.json', JSON.stringify(extracted, null, 2));
console.log(`Extracted ${extracted.length} new Skin1004 products.`);
