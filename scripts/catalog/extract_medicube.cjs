...
const targetBrand = 'Medicube';
const brandFile = path.join(__dirname, 'src/data/brands/medicube.ts');
...
scrapedFiles.forEach(file => {
  ...
  // Simpler regex to match the start and end of blocks
  // Then we filter by brand name inside the block
  const blocks = content.split(/(\{\s*id: \d+,)/);
  
  for (let i = 1; i < blocks.length; i += 2) {
    const block = blocks[i] + blocks[i+1];
    if (block.toLowerCase().includes("brand: 'medicube'") || block.toLowerCase().includes('brand: "medicube"')) {
      try {
...
fs.writeFileSync('extracted_medicube.json', JSON.stringify(extracted, null, 2));
console.log(`Extracted ${extracted.length} new Medicube products.`);
