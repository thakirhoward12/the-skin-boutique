...
// Heavy imports moved inside functions to prevent deployment timeout
...
export const syncOrderToShopify = onDocumentCreated(
    ...
    try {
        ...
        // We multiply the base USD price by 18.95 because the Shopify store is in ZAR
        const lineItems = orderData.items.map((item: any) => ({
            sku: (item.sku || item.id).toString(),
            title: item.name || item.title,
            price: (parseFloat(item.price) * ZAR_RATE).toFixed(2),
            quantity: item.quantity,
            requires_shipping: true
        }));

        // Extract unique suppliers for tagging
        const suppliers = [...new Set(orderData.items.map((i: any) => i.supplierId).filter(Boolean))];
        const supplierTags = suppliers.map(s => `Supplier_${s}`).join(', ');

        // Convert shipping price to ZAR as well
        const shippingPrice = (parseFloat(orderData.shipping || 0) * ZAR_RATE).toFixed(2);

        // 2. Map Firestore shipping address to Shopify shipping address
        const shippingAddress = orderData.shippingAddress || {};
        const customerName = orderData.customerName || '';
        
        // Removed Syncee auto-fulfill tag logic as per manual fulfillment strategy pivot
        ...
        const shopifyOrderPayload = {
            order: {
                ...
                tags: `TSB_Firebase, ${supplierTags}, ${orderData.paymentMethod.toUpperCase()}`,
